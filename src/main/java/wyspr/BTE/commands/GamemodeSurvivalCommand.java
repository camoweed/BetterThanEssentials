...
@SuppressWarnings("ALL") public class GamemodeSurvivalCommand implements CommandManager.CommandRegistry {
	@Override
	public void register(CommandDispatcher<CommandSource> commandDispatcher) {
		String[] literals = {"survival", "gms"};
	...
	private int playerArg(CommandContext<Object> context) throws CommandSyntaxException {
		...
		player.sendMessage(MessageFormat.format(
			"{0}Set gamemode to {1}Survival{0} for " + TextFormatting.RESET + user.getDisplayName(),
			TextFormatting.YELLOW,
			TextFormatting.CYAN
		));
		user.sendMessage(TextFormatting.YELLOW + "Your gamemode was set to " + TextFormatting.CYAN + "Survival");
		return 0;
	}
}
