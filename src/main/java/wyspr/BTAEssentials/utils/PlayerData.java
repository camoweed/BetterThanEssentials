...
public class PlayerData implements Serializable {
	...
	private final transient HashMap<String, TPARequestType> TPARequests;
...
//	private transient       int                             saveTick         = 0;
	...
	public void save() {
		...
		try {
			...
			BTAEssentials.LOGGER.error("Error writing file: {}", e.getMessage());
		}
	}
	...
	public static PlayerData get(Player player) {
		return ((IPlayerData) player).BTAEssentials$getPlayerData(player);
	}

	public static void set(Player player) {
		((IPlayerData) player).BTAEssentials$setPlayerData(player);
	}
...
//	}
	...
	public int TPCooldown() {
		...
		Instant TPAvailable = lastTPTime.plus(Duration.ofSeconds(BTAEssentials.TPTimeout));
	...
	public WorldPosition getLastPos() {
		return new WorldPosition(backPos.x, backPos.y, backPos.z, backPos.dimID);
	}
